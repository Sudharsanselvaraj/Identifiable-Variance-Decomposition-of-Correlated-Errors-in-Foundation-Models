# G3 — Minimum Valid Population (exclusion variant: deepseek_excluded)
...
Structural minimum n0 = 20 (identifiable: full rank + VIF<=10).
**Minimum VALID population = 22** of 45 (reduced).
Baseline (full available pool, n=45) at 300 reps: {'A': {'era_bias_pp': 0.12105515144532619, 'era_coverage_pct': np.float64(98.66666666666667), 'convergence_pct': 100.0}, 'B': {'era_bias_pp': -2.214693291773301, 'era_coverage_pct': np.float64(98.66666666666667), 'convergence_pct': 100.0}, 'n': 45} -> PASS strict bar; margin-confirmed at 1000 reps: A 0.51pp / B -1.81pp -> PASS.
Winner (n=22): margin-confirmed A 0.6pp / B -3.61pp — same order of magnitude as the full available pool (n=45), so the reduced population is nearly equivalent under the validation criterion at ~42% of the est. single-GPU cost.
...
Trace: the available pool (n=45) passes; the structural minimum n0 = 20 fails the confirmation margin (knife-edge on the bar); 22 passes — the extra model(s) over n0 are statistically necessary, not computationally convenient.

| n | models | rank | vif | pass | bias A | cov% A | bias B | cov% B | bias A conf | bias B conf |
|---|---|---|---|---|---|---|---|---|---|---|
| 45 | 45 | True | True | True | 0.12 | 98.7 | -2.21 | 98.7 | 0.51 | -1.81 |
| 20 | 20 | True | True | False | 3.74 | 95.0 | -5.48 | 97.3 | - | - |
...
| 22 | 22 | False | False | False | - | - | - | - | - | - |
| 22 | 22 | False | False | False | - | - | - | - | - | - |
| 22 | 22 | True | True | True | 2.11 | 96.7 | -4.0 | 98.7 | 0.6 | -3.61 |

Robustness at 2000 reps (same fixed seeds): A 1.23pp / B -3.20pp, era-share CI coverage 97.5% / 98.7%, convergence 100.0% / 100.0%.
