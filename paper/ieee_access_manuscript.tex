...
\begin{document}
\history{Date of submission August 2026.}
...
The difficulty is that the two explanations are not separable by inspection. Release year lies on the lineage-to-error path: a child model inherits both the errors of its parent and the training environment of its own release date. Release year is therefore a \emph{mediator} of the lineage path and a \emph{potential confounder} of the observational family--error association. Any single-number answer that tries to assign shared error ``to lineage'' or ``to era'' conflates the two roles of release year.
...
\begin{table*}[!t]
\caption{\textbf{TABLE~I: Related Work Comparison.}}
\label{tab:diff}
\centering
\begin{tabular}{|l|c|c|c|c|c|}
\hline
\textbf{Work} & \textbf{Correlated Error} & \textbf{Lineage} & \textbf{Era} & \textbf{Variance Decomp.} & \textbf{Ident. Gate} \\
\hline
Kim~\textit{et al.} (ICML 2025) & Agreement rate & --- & --- & --- & --- \\
\hline
...
\hline
Kuai~\textit{et al.} (2026) & Entanglement & --- & --- & --- & --- \\
\hline
Monoculture (Jo~\textit{et al.}, 2026) & Given input & --- & --- & --- & --- \\
\hline
\textbf{This work} & \textbf{Context} & \textbf{Variance comp.} & \textbf{Variance comp.} & \textbf{Crossed REML} & \textbf{Yes} \\
\hline
\end{tabular}
\end{table*}
...
\begin{table}[!t]
\caption{\textbf{TABLE~II: Estimands and Identification Strategy.}}
\label{tab:estimands}
\centering
\begin{tabular}{|p{55pt}|p{180pt}|p{155pt}|}
\hline
\textbf{Estimand} & \textbf{Definition} & \textbf{Identification Strategy} \\
\hline
$\theta_P$ (observational) & Lineage share conditional on era grouping: $\sigma^2_L / \sigma^2_T$ & Crossed family-era design with full rank, connected incidence graph \\
\hline
$\theta_M$ (mechanistic) & Lineage share with era held exactly fixed & Co-released family cohorts and verified cross-generation fine-tune chains; reported separately \\
\hline
\end{tabular}
\end{table}
...
\label{alg:g3}
\begin{algsteps}
\item \algkw{Input:} model set $\mathcal{M}$ ($|\mathcal{M}| = N$); occupancy $O$ (family $\times$ quarter); verified lineage edges $L$; per-model cost $c$; thresholds $\tau_{\text{bias}} = 4.0$pp, $\tau_{\text{cov}} = 90\%$, margin $\delta = 1$pp
...
\item \algind{1} $\text{Screen}(S, R=300)$: if $|b| > \tau_{\text{bias}}$ or $\kappa < \tau_{\text{cov}}$ then continue
...
TABLE~\ref{tab:d3} shows that all three detectors flag the aliasing in 100\% of repetitions with zero silent coverage. A nested design that appears to succeed is a broken detector, not a valid estimate. The threshold $1.9207$ is the $\chi^2_1$ 95\% critical value halved.
...
\hline
\textbf{Model} & \textbf{Family} & \textbf{Era} & \textbf{Accuracy} & \textbf{Fidelity} \\
\hline
...
\end{enumerate}
These features jointly induce one linearly dependent direction in the fitted design matrix. Full rank is a necessary but not sufficient condition: even designs that achieve full rank can fail the condition number gate.
...
\item Model-by-item error-similarity analysis is therefore not reported and is permanently deferred.
...
The negative result---that the 16-model population fails the identifiability gate---is informative. It demonstrates that the gate works as designed: it catches a non-identifiable design before misleading inferences are reported. The 5.4\% family share is not a substantive finding; it is a diagnostic output of an invalid design, an invalid-design diagnostic fit. The CI covering $[0\%, 100\%]$ is the honest expression of that invalidity.
...
received the degree from the SRM Institute of Science and Technology, Tiruchirappalli, India. He is currently with the Research Institute of IoT Cybersecurity, National Kaohsiung University of Science and Technology, Kaohsiung, Taiwan. His research interests include cybersecurity, IoT, and AI systems.
\end{IEEEbiographynophoto}
